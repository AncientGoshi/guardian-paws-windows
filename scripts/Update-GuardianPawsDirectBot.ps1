...
$agentAction = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument "-NoProfile -File `"$app\scripts\GuardianPaws.ChildAgent.ps1`" -StatePath `"$root\state.json`" -ExtensionUpdateUrl `"$ExtensionUpdateUrl`""
